    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateTemperatureFields() {
            if(tempCheck.checked) {
                ...
                // Limpa os campos de temperatura quando desmarcado
                document.querySelectorAll('[name="min_temperature"], [name="max_temperature"], [name="temperature_tolerance"], [name="temperature_control_type"], [name="temperature_unit"], [name="temperature_notes"]').forEach(field => {
                    if (field.tagName === 'SELECT') {
                        field.selectedIndex = 0;
                    } else {
                        field.value = field.name === 'temperature_tolerance' ? '1' : '';
                    }
                });
            }
        }
